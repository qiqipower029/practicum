...
OL_min = sapply(PM_data[1:71], function(x) min(x, na.rm = T))
...
```{r PCA for OL data}
# Calculate z-scores
mean_OL = mean_control = sapply(BA_data[1:71], function(x) mean(x)) 
sd_OL = sapply(BA_data[1:71], function(x) sd(x))
OL_data_z = OL_data
for(i in 1:71) {
...
for(i in 1:71) {
  OL_new[i+10] = OL_new[i+10]/sd_PM[i]
}
```
...
for(i in 1:481) {
  data = CL_log[i+10]
  a = unlist(data)
  model = clogit(asd ~ as.factor(birth_season) + as.factor(year_new) + sex_ab_cpairsampleselection1 + a, data = CL_log)

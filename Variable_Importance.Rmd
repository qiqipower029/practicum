...
coefficients = coef(model.lasso)
for (k in 2:257) {
  variable_lasso_BA$numbers[k-1] = ifelse(coefficients[k,] != "0", variable_lasso_BA$numbers[k-1]+1, variable_lasso$numbers[k-1])
}
...
# Sort the variable importance
sort_lasso_BA = variable_lasso[order(-variable_lasso$numbers),]
sort_rf_BA = variable_rf[order(-variable_rf$sum_mean_accuracy),]
...
coefficients = coef(model.lasso)
for (k in 2:184) {
  variable_lasso_PM$numbers[k-1] = ifelse(coefficients[k,] != "0", variable_lasso_PM$numbers[k-1]+1, variable_lasso$numbers[k-1])
}
...
# Sort the variable importance
sort_lasso_PM = variable_lasso[order(-variable_lasso$numbers),]
sort_rf_PM = variable_rf[order(-variable_rf$sum_mean_accuracy),]
variable_xgb_PM$sum_gain = as.numeric(variable_xgb_BA$sum_gain)
sort_xgb_PM = variable_xgb_PM[order(-variable_xgb_BA$sum_gain),]

# Join all the variable importance
# Join these three dataframes
lasso_rf_join = left_join(sort_lasso_PM, sort_rf_BA, by = "variable_names")
